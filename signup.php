...
if(!empty($_POST) && strlen($_POST['nom'])>2 && strlen($_POST['prenom'])>2 && strlen($_POST['entreprise'])>2 && strlen($_POST['poste'])>2 && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) && $_POST['password']===$_POST['confirm_password']){
	...
	$q = array('nom'=>$nom, 'prenom'=>$prenom, 'email'=>$email, 'password'=>$password, 'entreprise'=>$entreprise, 'poste'=>$poste,'token'=>$token);
	$sql = 'INSERT INTO users (nom, prenom, email, password, entreprise, poste, token) VALUES (:nom, :prenom, :email, :password, :entreprise, :poste, :token)';
	...
	<a   href="http://localhost/sopro/membresActions/activate.php?token='. $token .'&email=' .$to .'" >Activation du compte</a>';
	...
	$error_poste = 'L\'intitulé de votre poste doit contenir au minimum 2 caractères';
  ...
  <body>
	...
	<?php
		if(isset($error_nom)){
	 	...
	 	if((!isset($error_password)||isset($error_newpwd)||isset($error_oldpwd)||isset($error_entreprise)||isset($error_poste)) && !(empty($_POST)))
...
?>
	...
	<!-- Main hero unit for a primary marketing message or call to action -->
      <div class="hero-unit">
        ...
        <p><a class="btn btn-primary btn-large" href="infos.html">Plus d'infos &raquo;</a></p>
      </div>
